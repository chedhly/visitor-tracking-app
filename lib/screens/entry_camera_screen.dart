...
class EntryCameraScreen extends StatefulWidget {
  @override
  _EntryCameraScreenState createState() => _EntryCameraScreenState();
}

class _EntryCameraScreenState extends State<EntryCameraScreen> {
  ...
  Future<void> _captureAndProcess() async {
    ...
    try {
      ...
      if (plateNumber.isNotEmpty) {
        ...
        await _recordEntry(plateNumber);
      } else {
  ...
  Future<void> _recordEntry(String plateNumber) async {
    try {
      final visitorService = Provider.of<VisitorService>(context, listen: false);
      await visitorService.recordEntry(plateNumber);
      
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Vehicle $plateNumber entered - Barrier Opening'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Vehicle Entry')),
      body: Column(
        children: [
          Expanded(
